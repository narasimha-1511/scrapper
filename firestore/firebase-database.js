  ...
  hash() {
    if (this.lazyHash_ === null) {
      ...
      if (!this.priorityNode_.isEmpty()) {
        toHash +=
          "priority:" + priorityHashText(this.priorityNode_.val()) + ":";
      }
  ...
  hash() {
    if (this.lazyHash_ === null) {
      ...
      if (!this.getPriority().isEmpty()) {
        toHash +=
          "priority:" + priorityHashText(this.getPriority().val()) + ":";
      }
      this.forEachChild(PRIORITY_INDEX, (key, childNode) => {
...
const validateFirebaseMergeDataArg = function (fnName, data, path, optional) {
  ...
  if (!(data && typeof data === "object") || Array.isArray(data)) {
    throw new Error(
      errorPrefix$1 + " must be an object containing the children to replace."
    );
  }
  const mergePaths = [];
  each(data, (key, value) => {
...
class ChildEventRegistration {
  ...
  getEventRunner(eventData) {
    if (eventData.getEventType() === "cancel") {
      ...
      return () =>
        this.callbackContext.onValue(eventData.snapshot, eventData.prevName);
    }
  }
  matches(other) {
